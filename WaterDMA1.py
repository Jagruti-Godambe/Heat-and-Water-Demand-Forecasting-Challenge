...
# Returns 1 if the date is within the Daylight Saving Time period in Northern Denmark, otherwise 0.
def is_dst(dt, timezone="Europe/Copenhagen"):
    timezone = pytz.timezone(timezone)
    if dt.tzinfo is None:
        dt_aware = timezone.localize(dt)
    else:
        dt_aware = dt.astimezone(timezone)
    return 1 if dt_aware.dst() != timedelta(0) else 0
...
class Water_data_preprocessing:
    ...
    def basic_data_preprocessing(self, data, if_train_data=True):
      ...
      Water_data_with_meter_data = Water_data_with_meter_data.reset_index()
      Water_data_with_meter_data['hour'] = Water_data_with_meter_data['timestamp'].dt.hour
      Water_data_with_meter_data['day_of_week'] = Water_data_with_meter_data['timestamp'].dt.dayofweek
      Water_data_with_meter_data['month'] = Water_data_with_meter_data['timestamp'].dt.month
      Water_data_with_meter_data['is_weekend'] = Water_data_with_meter_data['day_of_week'].apply(lambda x: 1 if x >= 5 else 0)

      # Ensure the timestamps are in datetime format
      Water_data_with_meter_data['timestamp'] = pd.to_datetime(Water_data_with_meter_data['timestamp'])

     # Apply the DST function to create a new column in the Water_data_with_meter_data dataframe
      Water_data_with_meter_data['is_dst'] = Water_data_with_meter_data['timestamp'].apply(is_dst)
      if if_train_data:
        Water_data_with_meter_data['comsumption']= Water_data_with_meter_data['comsumption'].interpolate(method='linear')
        
      #calculate comsumption per year after interpolation
