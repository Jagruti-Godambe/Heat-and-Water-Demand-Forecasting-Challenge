...
class Heat_data_preprocessing:
    def __init__(self, data_path, meter_data_path, weather_data_path):
       
        self.train_data = Heat_data_preprocessing.load_heat_data(data_path)
        self.meter_data = Heat_data_preprocessing.load_meter_data(meter_data_path)
        self.Weather_data = Heat_data_preprocessing.load_weather_data(weather_data_path)
        self.scaler = MinMaxScaler()
        self.columns_to_scale = ['dew_point','feels_like','temp_min','temp_max','wind_speed',
                                 'wind_deg','clouds_all','temp','pressure','humidity']

    @staticmethod
    # Load and preprocess heat consumption data.
    def load_heat_data(data_path):
    ...
    def basic_data_preprocessing(self, data, if_train_data=True):
        # Merge heat data, meter data, and weather data
        Heat_data_with_meter_data = pd.merge(data, self.meter_data, on='timestamp', how='inner')
        Heat_data_with_meter_data = pd.merge(Heat_data_with_meter_data, self.Weather_data, on='timestamp', how='inner')

        # Interpolate consumption values if this is training data
        if if_train_data:
            Heat_data_with_meter_data['comsumption'] = Heat_data_with_meter_data['comsumption'].interpolate(method='linear')

        # Calculate consumption per meter after interpolation
        Heat_data_with_meter_data['Per_meter_comsumption_with_inter'] = Heat_data_with_meter_data['comsumption'] / Heat_data_with_meter_data['meters']
        
        # Drop unnecessary columns
        Heat_data_with_meter_data = Heat_data_with_meter_data.drop(['weather_id','timezone',
                                                                    'city_name','lat','lon','dt'], axis=1)
        return Heat_data_with_meter_data
    ...
    def transform(self, data_path):
        test_data = Heat_data_preprocessing.load_heat_data(data_path)
        Heat_data_with_meter_data = self.basic_data_preprocessing(test_data, if_train_data=False)
        Heat_data_with_meter_data[self.columns_to_scale] = self.scaler.transform(Heat_data_with_meter_data[self.columns_to_scale])
        Heat_data_with_meter_data = Heat_data_with_meter_data.set_index('timestamp')

        # Create one-hot encoded features for weather_icon
        dummies = pd.get_dummies(Heat_data_with_meter_data['weather_icon'],
                                 prefix='weather_icon', dtype='int')
        dummies = dummies[self.dummies_features]
        Heat_data_with_meter_data = pd.merge(Heat_data_with_meter_data, dummies, left_index=True, right_index=True)
        Heat_data_with_meter_data = Heat_data_with_meter_data.drop(['weather_main','weather_description','weather_icon'], axis=1)
    ...
    def get_columns_to_scale(self):
        """Return the list of columns that were scaled."""
...
def create_sequences(data, sequence_length_x, sequence_length_y, data_scaling_info):
    """Create sequences of input data and corresponding labels for training."""
    sequences = []
    labels = []
    scaling_prameters = []
    ...
    for i in tqdm(range(len(data) - (sequence_length_x + sequence_length_y))):
        sequence = data[i:i + sequence_length_x]
